...
from visualizaciones import bar_chart, dense_map, tweet_count, pie_chart, line_chart, heat_map, tree_map, dense_map, half_donut
...
##########################
    ...
    df['var_obj_cat'] = pd.cut(df['var_obj_cont'], bins, labels=names)
    ...
    st.subheader("Gráfico de conteo:")
    st.write(tweet_count(df))
    my_bar3.progress(0.125)
    time.sleep(0.2)

    st.subheader("Gráfico de barras:")
    st.write(bar_chart(df))
    my_bar3.progress(0.25)
    time.sleep(0.2)
    
    st.subheader("Gráfico de tarta:")
    st.write(pie_chart(df))
    my_bar3.progress(0.375)
    time.sleep(0.2)

    # st.subheader("Gráfico de líneas:")
    # st.write(line_chart(df))
    # my_bar3.progress(0.5)
    # time.sleep(0.2)

    st.subheader("Gráfico de calor:")
    st.write(heat_map(df))
    my_bar3.progress(0.625)
    time.sleep(0.2)

    st.subheader("Gráfico de árbol:")
    st.write(tree_map(df))
    my_bar3.progress(0.75)
    time.sleep(0.2)

    st.subheader("Gráfico de densidad:")
    st.write(dense_map(df))
    my_bar3.progress(0.875)
    time.sleep(0.2)

    st.subheader("Gráfico de donut:")
    st.write(half_donut(df))
    my_bar3.progress(1.0)
